...
Let $X$ be a complete Riemannian manifold, $V \to M$ a two dimensional vector bundle over an enlargeable manifold $M$. Assume that 
\begin{enumerate}[label=(\roman*)]
    \item There exists a proper map $f \colon X \to V$, $\mathrm{deg}(f) \neq 0$,
    \item \label{intro:eq:gromov_thm_condition_2}The total space of the circle bundle corresponding to $V$ is also enlargeable.
\end{enumerate}
Then 
\[
...
It is easy to see that condition \ref{intro:eq:gromov_thm_condition_2} is always fullfilled if the bundle $V \to M$ is trivial (compare Lemma \ref{enl:lem:constructing_enlargeable_mnf} \ref{enl:eq:lem_for_constructing_enlargeable_mnf_1}), which lead Gromov to pose the following question:
