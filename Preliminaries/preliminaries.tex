...
Let us begin by recalling the definition of enlargeability.

\begin{definition}
    A closed oriented Riemannian $n$-manifold $M$ is called enlargeable if for all $\epsilon >0$ there exists a finite Riemannian covering $\hat{M} \to M$ together with an $\epsilon$-contracting  map $\hat{M} \to S^n$ of non-zero degree.
\end{definition}

% (that is $\epsilon$-Lipschitz)

Note that this notion does not depend on the particular choice of Riemannian metric on $M$, since for closed manifolds all metrics are in bi-Lipschitz correspondence. Our definition of enlargeability is adopted from \cite{cecchini2021enlargeable} and \cite{HankeSchick} in contrast to the orginial definition in \cite{gromov1980spin}, where the coverings $\hat{M}$ were additionally required to be spin. The observation that enlargeability is an obstruction to the existence of positive scalar curvature metrics was first shown by Gromov and Lawson \cite{gromov1980spin} in the case where $M$ is spin. This was later generalized by Cecchini and Schick \cite{cecchini2021enlargeable} to the non-spin case.
...
\noindent\textbf{Facts:} \vspace{-.2cm}
\begin{enumerate}[label=(\roman*)] \itshape
    \item Products of enlargeable manifolds are enlargeable, \vspace{-.15cm}
    \item Let $M$ be closed oriented, $N$ enlargeable. Assume there exists a map $f \colon M \to N, \; \mathrm{deg}(f) \neq 0$. Then $M$ is enlargeable.
\end{enumerate}
